...
2.  [Enabling the components](#Enablingthecomponents)
3.  [Specifying repositories](#Repositories)
    1.  [Default Repository](#DefaultRepository)
    2.  [Repository Attributes](#RepositoryAttributes)
    3.  [Scoped Repository](#ScopedRepository)
    4.  [In the database](#ReposDatabase)
    5.  [In `trac.ini`](#ReposTracIni)
    6.  [In GitWeb](#ReposGitWeb)
4.  [Repository caching](#Repositorycaching)
5.  [Repository synchronization](#Synchronization)
    1.  [Explicit synchronization](#ExplicitSync)
    2.  [Per-request synchronization](#PerRequestSync)
6.  [Automatic changeset references in tickets](#CommitTicketUpdater)
7.  [Troubleshooting](#Troubleshooting)
    1.  [My trac-post-commit-hook doesn't work anymore](#trac-post-commit-hook)
    2.  [Git control files missing](#Gitcontrolfilesmissing)

## Quick start

+   Enable the repository connector(s) for the version control system(s) that you will use.
+   Add repositories through the *Repositories* admin page, using `trac-admin` or by editing the `[repositories]` section of [trac.ini](https://trac.streamboard.tv/oscam/wiki/TracIni#repositories-section).
...
The version control systems can be enabled by adding the following to the `[components]` section of your [trac.ini](https://trac.streamboard.tv/oscam/wiki/TracIni#components-section), or enabling the components through the *Plugins* admin page.
...
Trac supports multiple repositories per environment, and the repositories may be for different version control systems. Each repository must be defined in a repository configuration provider. Repository providers included with Trac are the [database store](https://trac.streamboard.tv/oscam/wiki/TracRepositoryAdmin#ReposDatabase), the [trac.ini configuration file](https://trac.streamboard.tv/oscam/wiki/TracRepositoryAdmin#ReposTracIni) and the [GitWeb configuration file](https://trac.streamboard.tv/oscam/wiki/TracRepositoryAdmin#ReposGitWeb). Additional providers are available as plugins.
...
| --- | --- |
| `alias` | Defines an alias to a real repository. All [TracLinks](https://trac.streamboard.tv/oscam/wiki/TracLinks) referencing the alias resolve to the aliased repository. Note that multiple indirection is not supported, so an alias must always point to a real repository. The `alias` and `dir` attributes are mutually exclusive. |
...
| `description` | The text specified in the `description` attribute is displayed below the top-level entry for the repository in the source browser. It supports [WikiFormatting](https://trac.streamboard.tv/oscam/wiki/WikiFormatting). |
...
| `sync_per_request` | When set to `true` the repository will be synchronized on every request (implicit synchronization). This is generally not recommended. See [repository synchronization](https://trac.streamboard.tv/oscam/wiki/TracRepositoryAdmin#Synchronization) for a comparison of explicit and implicit synchronization. The attribute defaults to `false`. |
| `type` | The `type` attribute specifies the version control system used by the repository. Trac provides support for Subversion and Git, and plugins add support for several other systems. If `type` is not specified, it defaults to the value of the `[versioncontrol]` [default\_repository\_type](https://trac.streamboard.tv/oscam/wiki/TracIni#versioncontrol-default_repository_type-option) option. |
...
The following [trac-admin](https://trac.streamboard.tv/oscam/wiki/TracAdmin) commands can be used to perform repository operations from the command line.
...
Repositories and repository attributes can be specified in the `[repositories]` section of [trac.ini](https://trac.streamboard.tv/oscam/wiki/TracIni#repositories-section). Every attribute consists of a key structured as `{name}.{attribute}` and the corresponding value separated with an equal sign (`=`). The name of the default repository is empty.

The main advantage of specifying repositories in `trac.ini` is that they can be inherited from a [global configuration](https://trac.streamboard.tv/oscam/wiki/TracIni#GlobalConfiguration). Cached repositories defined in `trac.ini` at the time of [environment initialization](https://trac.streamboard.tv/oscam/wiki/TracEnvironment#SourceCodeRepository) will be automatically synchronized if the repository's connector is enabled. One drawback is that due to limitations in the `ConfigParser` class used to parse `trac.ini`, the repository name is always lowercase.
...
[​GitWeb](https:https://git-scm.com/book/en/v2/Git-on-the-Server-GitWeb) is a CGI script that comes with Git for web-based visualization of repositories. Trac can read the gitweb-formatted `project.lists` file. The configuration is done through the `[gitweb-repositories]` section of [trac.ini](https://trac.streamboard.tv/oscam/wiki/TracIni#gitweb-repositories-section).

## Repository caching

Caching improves the performance browsing the repository, viewing logs and viewing changesets. Cached repositories must be [synchronized](https://trac.streamboard.tv/oscam/wiki/TracRepositoryAdmin#Synchronization), using either explicit or implicit synchronization. When searching changesets, only cached repositories are searched.

Repositories that support caching are cached by default. The Subversion and Git backends support caching. The [​Mercurial plugin](https:https://trac.edgewall.org/intertrac/TracMercurial "TracMercurial in The Trac Project") does not yet support caching ([​#8417](https:https://trac.edgewall.org/intertrac/%238417 "#8417 in The Trac Project")). To disable caching, set the `cached` attribute to `false`.
...
Either explicit or implicit synchronization can be used. Implicit synchronization is easier to configure, but may result in noticeably worse performance. The changeset added and modified events can't be triggered with implicit synchronization, so the [commit ticket updater](https://trac.streamboard.tv/oscam/wiki/TracRepositoryAdmin#Automaticchangesetreferencesintickets) won't be available.
...
Note that you may have to set the environment variable `PYTHON_EGG_CACHE` to the same value as was used for the web server configuration before calling `trac-admin`, if you changed it from its default location. See [TracPlugins](https://trac.streamboard.tv/oscam/wiki/TracPlugins#Settinguptheplugincache) for more information.

#### Subversion

##### Using `trac-svn-hook`

In a Unix environment, the simplest way to configure explicit synchronization is by using the [​contrib/trac-svn-hook](https:https://trac.edgewall.org/intertrac/source%3Abranches/1.4-stable/contrib/trac-svn-hook "source:branches/1.4-stable/contrib/trac-svn-hook in The Trac Project") script. `trac-svn-hook` starts `trac-admin` asynchronously to avoid slowing the commit and log editing operations. The script comes with a number of safety checks and usage advice. Output is written to a log file with prefix `svn-hooks-` in the environment `log` directory, which can make configuration issues easier to debug.
...
See the [​section about hooks](http://svnbook.red-bean.com/en/1.7/svn.reposadmin.create.html#svn.reposadmin.create.hooks) in the Subversion book for more information. Other repository types will require different hook configuration.

#### Git

[​Git hooks](https:https://git-scm.com/book/en/v2/Customizing-Git-Git-Hooks) can be used in the same way for explicit syncing of Git repositories.
...
For Mercurial, add the following entries to the `.hgrc` file of each repository accessed by Trac (if [​TracMercurial](https:https://trac.edgewall.org/intertrac/TracMercurial "TracMercurial in The Trac Project") is installed in a Trac `plugins` directory, download [​hooks.py](https:https://trac.edgewall.org/intertrac/source%3Amercurial-plugin/tracext/hg/hooks.py "source:mercurial-plugin/tracext/hg/hooks.py in The Trac Project") and place it somewhere accessible):
...
If the post-commit hooks are not available, the environment can be set up for per-request synchronization. The `sync_per_request` attribute for each repository in the database and in [trac.ini](https://trac.streamboard.tv/oscam/wiki/TracIni#trac-section) must be set to `true`.
...
This functionality requires installing a post-commit hook as described in [explicit synchronization](https://trac.streamboard.tv/oscam/wiki/TracRepositoryAdmin#ExplicitSync), and enabling the optional commit updater components through the *Plugins* admin panel or by adding the following line to the `[components]` section of your [trac.ini](https://trac.streamboard.tv/oscam/wiki/TracIni#components-section):

```
tracopt.ticket.commit_updater.* = enabled
```

For more information, see the documentation of the `CommitTicketUpdater` component in the *Plugins* admin panel and the [​CommitTicketUpdater](https:https://trac.edgewall.org/intertrac/CommitTicketUpdater "CommitTicketUpdater in The Trac Project") page.

## Troubleshooting

### My trac-post-commit-hook doesn't work anymore

You must now use the optional components from `tracopt.ticket.commit_updater.*`, which you can activate through the *Plugins* admin page, or by directly modifying the [\[components\]](https://trac.streamboard.tv/oscam/wiki/TracIni#components-section) section in the trac.ini. Be sure to use [explicit synchronization](https://trac.streamboard.tv/oscam/wiki/TracRepositoryAdmin#ExplicitSync).

See [​CommitTicketUpdater#Troubleshooting](https:https://trac.edgewall.org/intertrac/CommitTicketUpdater%23Troubleshooting "CommitTicketUpdater#Troubleshooting in The Trac Project") for more troubleshooting tips.
...
[Last modified](https://trac.streamboard.tv/oscam/wiki/TracRepositoryAdmin?action=diff&version=7 "Version 7 by trac") [2 years ago](https://trac.streamboard.tv/oscam/timeline?from=2020-06-07T14%3A06%3A46%2B02%3A00&precision=second "See timeline at 06/07/20 14:06:46") Last modified on 06/07/20 14:06:46
