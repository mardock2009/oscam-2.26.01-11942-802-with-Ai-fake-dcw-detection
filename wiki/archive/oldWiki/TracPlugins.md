...
1.  [Plugin discovery](#Plugindiscovery)
2.  [Installing a Trac plugin](#InstallingaTracplugin)
    1.  [For a single project](#Forasingleproject)
        1.  [Uninstalling](#Uninstalling)
    2.  [For all projects](#Forallprojects)
        1.  [Using pip](#Usingpip)
        2.  [From PyPI](#FromPyPI)
        3.  [From source](#Fromsource)
        4.  [Enabling the plugin](#Enablingtheplugin)
        5.  [Upgrading the environment](#Upgradingtheenvironment)
        6.  [Redeploying static resources](#Redeployingstaticresources)
        7.  [Uninstalling](#Uninstalling1)
3.  [Web-based plugin administration](#Web-basedpluginadministration)
4.  [Setting up the plugin cache](#Settinguptheplugincache)
    1.  [About hook scripts](#Abouthookscripts)
5.  [Writing Trac Plugins](#WritingTracPlugins)
6.  [Troubleshooting](#Troubleshooting)
    1.  [Did you get the correct version of the Python egg?](#DidyougetthecorrectversionofthePythonegg)
    2.  [Is the plugin enabled?](#Isthepluginenabled)
    3.  [Check the permissions](#Checkthepermissions)
    4.  [Check the log files](#Checkthelogfiles)
    5.  [Verify you have the proper permissions](#Verifyyouhavetheproperpermissions)
    6.  [Is the wrong version of the plugin loading?](#Isthewrongversionofthepluginloading)
    7.  [If all of the above failed](#Ifalloftheabovefailed)

# Trac plugins

Trac is extensible with [​plugins](https:https://trac.edgewall.org/intertrac/PluginList "PluginList in The Trac Project"). Plugin functionality is based on the [​component architecture](https:https://trac.edgewall.org/intertrac/TracDev/ComponentArchitecture "TracDev/ComponentArchitecture in The Trac Project"), described in the [​plugin development](https:https://trac.edgewall.org/intertrac/TracDev/PluginDevelopment "TracDev/PluginDevelopment in The Trac Project") page.

## Plugin discovery

From the user's point of view, a plugin is either a single .py file or a package (.egg or .whl). Trac looks for plugins in Python's `site-packages` directory, the [global shared](https://trac.streamboard.tv/oscam/wiki/TracIni#GlobalConfiguration) `plugins` directory and the [project environment](https://trac.streamboard.tv/oscam/wiki/TracEnvironment) `plugins` directory. Plugins installed to the project environment `plugins` directory are enabled, unless explicitly disabled in the `[components]` section of the `trac.ini` file. Plugins installed elsewhere must be explicitly enabled in the [\[components\]](https://trac.streamboard.tv/oscam/wiki/TracIni#components-section) section of the `trac.ini` file.

## Installing a Trac plugin

The instructions below are applicable to a plugins installed as packages. Plugins implemented as a single `py` file should be downloaded and copied to the [project environment](https://trac.streamboard.tv/oscam/wiki/TracEnvironment) `plugins` directory or the [global shared](https://trac.streamboard.tv/oscam/wiki/TracIni#GlobalConfiguration) plugins directory.
...
+   Copy the egg file to the `plugins` directory of the [project environment](https://trac.streamboard.tv/oscam/wiki/TracEnvironment).

Make sure the web server has sufficient permissions to read the plugin egg and restart the web server. If you are running as a ["tracd" standalone server](https://trac.streamboard.tv/oscam/wiki/TracStandalone), restart tracd (i.e. kill the process and run again).

Trac also searches for plugins installed in the [global shared](https://trac.streamboard.tv/oscam/wiki/TracIni#GlobalConfiguration) plugins directory. This is a convenient way to share the installation of plugins across several, but not all, environments.
...
The modern Python package manager, `pip`, is included in Python 2.7.9 and later. In earlier versions of Python it can be installed through the package manager of your OS (e.g. `apt-get install python-pip`) or using [​get\_pip.py](https:https://pip.pypa.io/en/latest/installing.html#install-pip).

Using `pip`, the plugin will be installed in the [​wheel format](https:https://pythonwheels.com/), which is the modern standard for Python and a replacement for the egg format.

#### From PyPI

Some plugins, such as \[th:TagsPlugin TracTags\], can be installed directly from [​PyPI](https:https://pypi.org/) using `pip`:
...
Plugins can also be enabled from the [administration](https://trac.streamboard.tv/oscam/wiki/TracPlugins#Web-basedpluginadministration) page.
...
If you [mapped static resources](https://trac.streamboard.tv/oscam/wiki/TracInstall#MappingStaticResources) so they are served by the web server, and the plugin contains static resources (CSS, JavaScript and image files), the resources will need to be deployed to the location on the filesystem that is served by the web server.

Execute the `deploy` command, as is done during install and [upgrade](https://trac.streamboard.tv/oscam/wiki/TracUpgrade#a5.Refreshstaticresources):
...
This works whether you're using the [CGI](https://trac.streamboard.tv/oscam/wiki/TracCgi) or the [mod\_python](https://trac.streamboard.tv/oscam/wiki/TracModPython) front-end. Put this directive next to where you set the path to the [Trac environment](https://trac.streamboard.tv/oscam/wiki/TracEnvironment), i.e. in the same `<Location>` block.
...
For [FastCGI](https://trac.streamboard.tv/oscam/wiki/TracFastCgi), you'll need to `-initial-env` option, or whatever is provided by your web server for setting environment variables.
...
+   [​Developer documentation](https:https://trac.edgewall.org/intertrac/TracDev "TracDev in The Trac Project")
+   [​Examples on trac-hacks.org](https:https://trac-hacks.org/)
+   [​sample-plugins](https:https://trac.edgewall.org/intertrac/browser%3Abranches/1.4-stable/sample-plugins "browser:branches/1.4-stable/sample-plugins in The Trac Project")
...
If you install a plugin globally, i.e. *not* inside the `plugins` directory of the Trac project environment, you must explicitly enable it in [trac.ini](https://trac.streamboard.tv/oscam/wiki/TracIni). Make sure that:
...
See [​TracTroubleshooting#ChecktheLogs](https:https://trac.edgewall.org/intertrac/TracTroubleshooting%23ChecktheLogs "TracTroubleshooting#ChecktheLogs in The Trac Project").
...
See [TracSupport](https://trac.streamboard.tv/oscam/wiki/TracSupport).

* * *

See also [TracGuide](https://trac.streamboard.tv/oscam/wiki/TracGuide), [TracIni](https://trac.streamboard.tv/oscam/wiki/TracIni).

[Last modified](https://trac.streamboard.tv/oscam/wiki/TracPlugins?action=diff&version=8 "Version 8 by trac") [2 years ago](https://trac.streamboard.tv/oscam/timeline?from=2020-06-07T14%3A06%3A46%2B02%3A00&precision=second "See timeline at 06/07/20 14:06:46") Last modified on 06/07/20 14:06:46
