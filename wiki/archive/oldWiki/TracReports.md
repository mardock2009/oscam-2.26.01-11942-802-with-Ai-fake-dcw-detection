...
Rather than have its own report definition format, [TracReports](https://trac.streamboard.tv/oscam/wiki/TracReports) relies on standard SQL `SELECT` statements for custom report definition.

> **Note:** The report module is being phased out in its current form because it seriously limits the ability of the Trac team to make adjustments to the underlying database schema. We believe that the [query module](https://trac.streamboard.tv/oscam/wiki/TracQuery) is a good replacement that provides more flexibility and better usability. While there are certain reports that cannot yet be handled by the query module, we intend to further enhance it so that at some point the reports module can be completely removed. This also means that there will be no major enhancements to the report module anymore.

> You can already completely replace the reports module by the query module simply by disabling the former in [trac.ini](https://trac.streamboard.tv/oscam/wiki/TracIni):
>
> ```
> [components]
...
+   **Description** — A brief description of the report, in [WikiFormatting](https://trac.streamboard.tv/oscam/wiki/WikiFormatting) text.
...
All reports support syndication using XML/RSS 2.0. To subscribe to an RSS feed, click the orange 'XML' icon at the bottom of the page. See [TracRss](https://trac.streamboard.tv/oscam/wiki/TracRss) for general information on RSS support in Trac.

## Creating Custom Reports

Creating a custom report requires a comfortable knowledge of SQL.

Note that you need to set up [permissions](https://trac.streamboard.tv/oscam/wiki/TracPermissions#Reports) in order to see the buttons for adding or editing reports.
...
See [TracTickets](https://trac.streamboard.tv/oscam/wiki/TracTickets) for a detailed description of the column fields.
...
For more flexible reports, Trac supports the use of *dynamic variables* in report SQL statements. In short, dynamic variables are *special* strings that are replaced by custom data before query execution. Dynamic variables are entered through the preferences form and the values are autocompleted *(Since 1.3.2)*.
...
The value of the dynamic variable can be assigned in the report preferences form.

To assign a value to `$PRIORITY` in the URL for a report, leave out the leading `$`:
...
It is possible to assign a default value to the variable, within a SQL comment:
...
To format reports, [TracReports](https://trac.streamboard.tv/oscam/wiki/TracReports) looks for 'magic' column names in the query result. These 'magic' names are processed and affect the layout and style of the final report.
...
Those columns can also be defined but marked as hidden, see [below](https://trac.streamboard.tv/oscam/wiki/TracReports#column-syntax).

See [​trac:wiki/CookBook/Configuration/Reports](https:https://trac.edgewall.org/intertrac/wiki/CookBook/Configuration/Reports "wiki/CookBook/Configuration/Reports in The Trac Project") for some example of creating reports for realms other than *ticket*.
...
If you have added custom fields to your tickets (see [TracTicketsCustomFields](https://trac.streamboard.tv/oscam/wiki/TracTicketsCustomFields)), you can write a SQL query to cover them. You'll need to make a join on the ticket\_custom table, but this isn't especially easy.

If you have tickets in the database *before* you declare the extra fields in trac.ini, there will be no associated data in the ticket\_custom table. To get around this, use SQL's "LEFT OUTER JOIN" clauses. See [​TracIniReportCustomFieldSample](https:https://trac.edgewall.org/intertrac/TracIniReportCustomFieldSample "TracIniReportCustomFieldSample in The Trac Project") for some examples.
...
1.  [changing the sort order](https://trac.streamboard.tv/oscam/wiki/TracReports#sort-order)
...
See also: [TracTickets](https://trac.streamboard.tv/oscam/wiki/TracTickets), [TracQuery](https://trac.streamboard.tv/oscam/wiki/TracQuery), [TracGuide](https://trac.streamboard.tv/oscam/wiki/TracGuide), [​Query Language Understood by SQLite](http://www.sqlite.org/lang_expr.html)

[Last modified](https://trac.streamboard.tv/oscam/wiki/TracReports?action=diff&version=9 "Version 9 by trac") [2 years ago](https://trac.streamboard.tv/oscam/timeline?from=2020-06-07T14%3A06%3A46%2B02%3A00&precision=second "See timeline at 06/07/20 14:06:46") Last modified on 06/07/20 14:06:46
